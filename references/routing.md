...
Model choice is independent of effort. Selection order is: explicit user constraint, catalog `routing_tier`/`capability_tier`/`quality_tier`, recognizable family-tier names, a generic balanced model, the current configured model, then catalog priority. Name matching is therefore only a fallback. Catalog metadata and explicit user constraints win.
